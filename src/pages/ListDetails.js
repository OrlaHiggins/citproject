...
const ListDetails = () => {
    ...
    const findCheapestStore = async () => {
      if (listDetails && productDetails) {
        ...
        for (const item of listDetails.items) {
          ...
          if (product) {
            const response = await axios.get(`http://localhost:5432/products?category=${product.category}`);
  ...
  const handleAddItem = async (productId) => {
    ...
    if (product) {
      const newItem = { title: product.product, productId: product._id, price: product.price };
      const updatedList = { ...editedList };
      ...
      try {
        const token = window.localStorage.getItem('token');
        const response = await axios.put(`http://localhost:5432/lists/${listId}`, updatedList, {
          headers: {
...
<div className="list-details-container">
  ...
  <div className="list-details-content">
    <div className="list-details-section">
      ...
      <div className={`product-grid ${window.innerWidth > 1200 ? 'large-screen' : ''}`}>
      ...
      <p className="total-price">Total Price: £{editedList.totalPrice.toFixed(2)}</p>
    </div>
    {cheapestStore && (
      <div className="list-details-section">
        <h3 className="section-heading">Cheapest Store: {cheapestStore.storeId}</h3>
        <div className={`product-grid ${window.innerWidth > 1200 ? 'large-screen' : ''}`}>
        ...
        <p className="total-price">Total Price: £{cheapestStore.totalPrice.toFixed(2)}</p>
      </div>
    )}
  </div>
      {showEditModal && (
      <div className="edit-modal">
        <div className="edit-modal-content">
          ...
          <ul>
            {editedList.items.map((item, index) => (
              <li key={index}>
                {item.title} - £{(productDetails[item.productId]?.price || 0).toFixed(2)}
